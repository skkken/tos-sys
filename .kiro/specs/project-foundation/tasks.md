...
- [ ] 1. 依存パッケージのインストールと shadcn/ui 初期化
- [ ] 1.1 Supabase、Zod、date-fns、@react-pdf/renderer の依存パッケージを追加する
...
- [ ] 5.2 (P) Zod スキーマによるバリデーションヘルパーと共通スキーマを作成する
...
- [ ] 5.3 (P) 日本語ロケールの通貨・日付フォーマットユーティリティを作成する
...
- [ ] 7. 型定義と DB マイグレーション
- [ ] 7.1 (P) Supabase 型定義のプレースホルダーを作成する
  - 型定義ファイルのプレースホルダーを配置
  - Supabase CLI での型生成コマンドをコメントで文書化
  - _Requirements: 10.1, 10.2_

- [ ] 7.2 (P) 会社設定テーブルのマイグレーションを作成する
...
- [ ] 8. ビルド検証
- [ ] 8.1 `pnpm build` を実行し、全コンポーネントがエラーなくビルドされることを確認する
  - 型エラーがないことを確認
